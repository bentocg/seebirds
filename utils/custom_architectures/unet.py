...
class UNet(nn.Module):
    def __init__(self, scale=32, n_channels=3, n_classes=3, drop_rate=0.5, threshold=0.07, max_n = 50):
    ...
    def forward(self, x):
        ...
        x = self.up4(x, x1)
        x = self.outc(x)
        count = torch.sum(self.sigmoid(x), axis=[2,3])
      
        # return output dict
        return {'count': torch.squeeze(count),
                'heatmap': x}
